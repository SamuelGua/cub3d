...
/*   Updated: 2024/05/30 11:39:20 by scely            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	castray(t_data *data)
{
	...
	while (++x < SCREEN_W)
	{
		...
		if (side == 0)
			perpwall_dist = (map_x - data->ray.pos_x + (1 - step_x) / 2)
				/ raydir_x;
		else
			perpwall_dist = (map_y - data->ray.pos_y + (1 - step_y) / 2)
				/ raydir_y;
		// 3
